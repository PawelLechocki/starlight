...
describe("WWWShield", async function () {
		...
		describe("First call", async function () {
			...
			it("should call add", async () => {
				try {
					...
					await startEventFilter("WWWShield");
					...
					const { tx, encEvent } = await add(186);
					// prints the tx
					console.log(tx);
			...
			it("should update the merkle tree", async () => {
				try {
					...
					const path = await getSiblingPath("WWWShield", leafIndex);
